...
Visando ser o menos intrusivo possível para os usuários dos navegadores, a solução aqui proposta é baseada em um intermediador (proxy\footnote{Um proxy é um conceito da computação utilizada em diversos contextos. Este é um intermediador entre duas entidades. Supondo que a entidade A deseja se comunicar com a entidade B, em alguns casos é mais vantajoso A falar com uma outra entidade C (proxy) e este se comunicar com B. Exemplos das necessidades de um proxy são quando A não tem conexão com B ou quando A não sabe falar a linguagem de B.}). A solução não requer alteração no código fonte de nenhum navegador existente ou a criação de um navegador que se adequa a nossa solução. O uso do WebP2P será transparente para os usuários caracterizando-a como pouco intrusiva.

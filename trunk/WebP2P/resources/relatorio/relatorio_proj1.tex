...
Acreditamos que as soluções descritas acima não são adequadas para alguns sites em determinados momentos. Um exemplo é uma página que disponibiliza o resultado de um concurso, o número de acessos no momento da divulgação atinge um estado no qual o site não suporta todas as requisições. Isso traz problemas tanto para os usuários, que não conseguem acessar o conteúdo, como também para o administrador, que pode vir a sofrer prejuízos financeiros e/ou reputacionais.
...
\subsection{Coral Content Distribution Network}
...
\subsection{Browsers-Aware Proxy Server}
